...
class Wsi_Region(Dataset):
    ...
    def __init__(self, wsi_object, top_left=None, bot_right=None, level=0, 
        ...
        # Debugging log for contours_tissue
        print(f"Number of contours in tissue: {len(wsi_object.contours_tissue)}")

        filtered_coords = []
        # Iterate through tissue contours for valid patch coordinates
        for cont_idx, contour in enumerate(wsi_object.contours_tissue): 
            print(f"Processing contour {cont_idx + 1}/{len(wsi_object.contours_tissue)}")
            ...
            coord_results, _ = wsi_object.process_contour(
                contour, wsi_object.holes_tissue[cont_idx], level, '', 
        ...
        # Handle empty filtered_coords
        if not filtered_coords:
            raise ValueError("No valid coordinates found. Check the contours or input parameters.")
