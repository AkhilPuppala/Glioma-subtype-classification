...
def infer_single_slide(model, features, label, reverse_label_dict, k=1):
    ...
    with torch.inference_mode():
        if isinstance(model, (CLAM_SB, CLAM_MB)):
            # model(features) returns either a dict or tuple depending on implementation.
            # we call model(features) and then explicitly unpack so behaviour is consistent.
            logits, Y_prob, Y_hat, A, _ = model(features)
            Y_hat = int(Y_hat.item())

            # For multi-branch (MB) models we keep only the attention for predicted class
            ...
            raise NotImplementedError("Model type not supported in infer_single_slide")
...
if __name__ == '__main__':
    ...
    # Use computed step_size here (not patch_size)
    blocky_wsi_kwargs = {
        'top_left': None,
        'bot_right': None,
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        if hasattr(heatmap_args, 'use_roi') and heatmap_args.use_roi:
        ...
        # For PNG/single-level images, level_downsamples may be [1]. This will be handled safely.
        try:
            wsi_ref_downsample = wsi_object.level_downsamples[patch_args.patch_level]
        except Exception:
            # fallback to 1 if the requested patch_level does not exist (common for PNG)
        ...
        # handle vis_level safely for PNGs / non-pyramidal slides
        if vis_params.get('vis_level', -1) < 0:
            try:
                best_level = wsi_object.wsi.get_best_level_for_downsample(32)
                vis_params['vis_level'] = best_level
            except Exception:
                # Non-pyramidal images (PNG) or other errors -> use level 0
        ...
        # Ensure top_left and bot_right are defined in blocky_wsi_kwargs
        blocky_wsi_kwargs['top_left'] = blocky_wsi_kwargs.get('top_left', (0, 0))
        blocky_wsi_kwargs['bot_right'] = blocky_wsi_kwargs.get('bot_right', None)

        # Debugging log to verify wsi_kwargs
        print("blocky_wsi_kwargs:", blocky_wsi_kwargs)
