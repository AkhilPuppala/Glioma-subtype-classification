...
def get_cam_1d(classifier, features):
	# robustly find classifier weight and compute CAM maps
	# support DataParallel
	mod = classifier.module if hasattr(classifier, 'module') else classifier
	...
	for p in mod.parameters():
		if p.dim() == 2 and p.shape[1] == feat_dim:
			weight = p
			break

	# fallback to state_dict search
	if weight is None:
		for k, v in mod.state_dict().items():
			if 'weight' in k and v.dim() == 2 and v.shape[1] == feat_dim:
				weight = v
				break

	# last resort: first 2-D param
	if weight is None:
		for p in mod.parameters():
			if p.dim() == 2:
				weight = p
	...
	if weight is None:
		raise RuntimeError('Could not locate a 2-D weight matrix in classifier for CAM computation')

	if features.dim() == 2:
		features = features.unsqueeze(0)

	cam_maps = torch.einsum('bgf,cf->bcg', [features, weight.to(features.device)])
	return cam_maps
