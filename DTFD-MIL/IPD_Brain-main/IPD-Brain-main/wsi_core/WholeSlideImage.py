...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            if len(asset_dict) > 0:
                if init:
                    # write initial hdf5 and record the hdf5 file path for this WSI
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        # if we have more than one coordinate, create the asset dict so patches/.h5 can be written
        # (require >1 to match original behavior that avoided single-point hdf5 writes)
        if len(results) > 1:
