...
def main(params):
    ...
    for ii in range(params.EPOCH):
        ...
        # ---- compute val_acc using second-tier outputs (gPred_1, gt_1)
        try:
            ...
            if vp.ndim == 2:
                ...
                vp = vp.ravel().astype(int)
            vg = vg.ravel().astype(int)
            L = min(len(vp), len(vg))
            val_acc = float(np.mean(vp[:L] == vg[:L]))
        except Exception as e:
            print("Val ACC calc error:", e)
            val_acc = 0.0
        ...
        # Save best by AUC (existing logic)
        if auc_val > best_auc:
            ...
            if params.isSaveModel:
                try:
                    best_ckpt = {
                        ...
                        'classifier': classifier.state_dict(), 'dim_reduction': dimReduction.state_dict(),
                        'attention': attention.state_dict(), 'att_classifier': attCls.state_dict()
                    }
    ...
    # FINAL test acc (use second-tier final_preds/final_gts)
    test_acc = 0.0
    try:
        ...
        vg = vg.ravel().astype(int)
        L = min(len(vp), len(vg))
        test_acc = float(np.mean(vp[:L] == vg[:L]))
    except Exception:
        test_acc = 0.0
